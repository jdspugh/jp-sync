...
function rsync(dirs,hosts) {
  ...
  hosts.forEach(h=>{
    ...
    if (s.stderr.length>0) console.error('rsync stderr: '+s.stderr)
    D('rsync stdout: '+s.stdout)
  })
}
...
function watch(d,r,n) {// params: directory array, remote hosts, restart nginx?
  ...
  rsync([d],r)
  const s=spawn('jp-watch',[d],{uid:0,gid:0})
  s.stdout.on('data',d=>{// results returned in '\n' separated batches
    D('jp-watch stdout.on(data) d='+d)
